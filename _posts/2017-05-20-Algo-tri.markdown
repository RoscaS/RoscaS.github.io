...
#include<iostream>
#include<cmath>
#include"..//..//00Libs//queue.hpp"
using namespace std;

void debug(Queue<int> **queue, int unite);
int findMax(int *t, int g, int d);
int nbPasses(int *t, int g, int d);
int extraireUnite(int n, int pos);
void triBase(int *t, int g, int d);

int main()
{   
    int liste[] = {3221,1,10,9680,577,9420,7,5622,4793,2030,3138,82,2599,743,4127};
    int gauche  = 0;
    int droite  = 14;

    cout << "\ntableau initial: ";
    for (auto &x: liste) { cout << x << " "; }

    triBase(liste,gauche,droite);
...
{
    cout << "\n\nUNITE " << unite << "\n";
    for (int casier = 0; casier <= 9; casier++)
    {
        cout << "\tcasier " << casier << "\n\t\t";
        queue[casier]->display();
    }
}
...
int nbPasses(int *t, int g, int d)
{
    return log10(findMax(t,g,d))+1;
}


int extraireUnite(int n, int pos)
{
    return (static_cast<int>(n/pow(10,pos))%10);
}
...
{
    // initialisation
    int digits = nbPasses(t,g,d);
    Queue<int> **qTab = new Queue<int>*[10];

    for (int casier = 0; casier <= 9; ++casier)
    {
        qTab[casier] = new Queue<int>;
    }    

    // tri
    for (int unite = 0; unite < digits; ++unite)
    {

        for (int index = g; index <= d; ++index)
        {
            qTab[extraireUnite(t[index], unite)]->enqueue(t[index]);
        }
        ...
        for (int casier = 0; casier <= 9; casier++)
        {
            while (qTab[casier]->isEmpty() == false)
            {
                t[indexe] = qTab[casier]->dequeue();
                ++indexe;
            }
        }    
    }
