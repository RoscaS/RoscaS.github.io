...
Il nous faut dans un premier temps créer un export de la base de données de Stackoverflow.

```export
SELECT TOP 100 p.CreationDate, p.id as [Post Link]
FROM Post p
...
Cette façon de faire n'est vraiment pas pratique (réutilisable). On peut commencer à en faire une fonction réutilisable.
